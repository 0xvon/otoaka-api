...
struct ExternalController: RouteCollection {
    func boot(routes: RoutesBuilder) throws {
        try routes.on(endpoint: Endpoint.CheckGlobalIP.self, use: { req, uri in
        ...
        try routes.on(endpoint: Endpoint.NotifyUpcomingLives.self, use: { req, uri in
        ...
        try routes.on(endpoint: Endpoint.SendNotification.self, use: { req, uri in
        ...
        try routes.on(endpoint: Endpoint.NotifyPastLives.self, use: { req, uri in
            let liveRepository = Persistance.LiveRepository(db: req.db)
            let notificationService = makePushNotificationService(request: req)
        ...
        try routes.on(endpoint: ScanGroups.self, use: { req, uri in
