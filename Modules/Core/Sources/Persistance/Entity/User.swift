...
final class User: Model {
    ...
    init(
        ...
        biography: String?, sex: String?, age: Int?, liveStyle: String?, residence: String?,
        thumbnailURL: String?, role: Domain.RoleProperties, twitterUrl: URL?, instagramUrl: URL?
    ) {
...
extension Endpoint.UserFeed {
    ...
    > {
        ...
        return id.and(author).and(group)
            ...
            .map {
                Endpoint.UserFeed(
                    id: ID($0), text: entity.text, feedType: feedType, author: $1,
                    ogpUrl: entity.ogpUrl, thumbnailUrl: entity.thumbnailUrl, group: $2,
                    title: entity.title, createdAt: entity.createdAt!)
            }
    }
}
...
extension Endpoint.Post {
    static func translate(fromPersistance entity: Post, on db: Database) -> EventLoopFuture<
        Endpoint.Post
    > {
        let eventLoop = db.eventLoop
        let id = eventLoop.submit { try entity.requireID() }
...
extension Endpoint.PostTrack {
    static func translate(fromPersistance entity: PostTrack, on db: Database) -> EventLoopFuture<
        Endpoint.PostTrack
    > {
        let eventLoop = db.eventLoop
        let id = eventLoop.submit { try entity.requireID() }
        ...
        return id.map { $0 }
            .map {
                Endpoint.PostTrack(
                    id: ID($0), trackName: entity.trackName, groupName: entity.groupName,
                    type: type, thumbnailUrl: entity.thumbnailUrl)
            }
    }
}

extension Endpoint.PostComment {
    static func translate(fromPersistance entity: PostComment, on db: Database) -> EventLoopFuture<
        Endpoint.PostComment
    > {
        let author = entity.$author.get(on: db).flatMap {
            Endpoint.User.translate(fromPersistance: $0, on: db)
        }
        let post = entity.$post.get(on: db).flatMap {
            Endpoint.Post.translate(fromPersistance: $0, on: db)
        }

        return author.and(post)
            .map { ($0, $1) }
            .flatMapThrowing {
                try Endpoint.PostComment(
                    id: .init(entity.requireID()), text: entity.text, author: $0, post: $1,
                    createdAt: entity.createdAt!)
            }
    }
}
...
extension Endpoint.UserNotification {
    static func translate(fromPersistance entity: UserNotification, on db: Database)
        ...
        case .follow:
            ...
            return user.and(followedBy).map { ($0, $1) }
                .flatMapThrowing { user, followedBy in
                    return try Endpoint.UserNotification(
                        id: .init(entity.requireID()), user: user, isRead: entity.isRead,
                        notificationType: .follow(followedBy), createdAt: entity.createdAt!)
                }
        case .like:
                ...
                .flatMapThrowing { user, likedBy, likedFeed in
                    return try Endpoint.UserNotification(
                        ...
                        notificationType: .like(
                            Endpoint.UserFeedLike(feed: likedFeed, likedBy: likedBy)),
                        createdAt: entity.createdAt!)
                }
        case .like_post:
            let likedBy = entity.$likedBy.get(on: db)
                ...
                .flatMapThrowing { user, likedBy, likedPost in
                    return try Endpoint.UserNotification(
                        ...
                        notificationType: .likePost(
                            Endpoint.PostLike(post: likedPost, likedBy: likedBy)),
                        createdAt: entity.createdAt!)
                }
        case .comment:
            ...
            return user.and(comment).map { ($0, $1) }
                .flatMapThrowing { user, comment in
                    return try Endpoint.UserNotification(
                        id: .init(entity.requireID()), user: user, isRead: entity.isRead,
                        notificationType: .comment(comment), createdAt: entity.createdAt!)
                }
        case .comment_post:
            ...
            return user.and(comment).map { ($0, $1) }
                .flatMapThrowing { user, comment in
                    return try Endpoint.UserNotification(
                        id: .init(entity.requireID()), user: user, isRead: entity.isRead,
                        notificationType: .postComment(comment), createdAt: entity.createdAt!)
                }
        case .official_announce:
            return user.flatMapThrowing { user in
                return try Endpoint.UserNotification(
                    id: .init(entity.requireID()), user: user, isRead: entity.isRead,
                    notificationType: .officialAnnounce(
                        Endpoint.OfficialAnnounce(title: entity.title!, url: entity.url)),
                    createdAt: entity.createdAt!)
            }
        }
    }
