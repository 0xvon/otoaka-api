...
extension Endpoint.MessageRoom {
    static func translate(fromPersistence entity: MessageRoom, on db: Database) -> EventLoopFuture<
        Endpoint.MessageRoom
    > {
        ...
        let members = entity.$members.query(on: db).all().flatMapEach(on: db.eventLoop) {
            member -> EventLoopFuture<User> in
            return member.$user.get(on: db)
        }
...
extension Endpoint.Message {
    static func translate(fromPersistence entity: Message, on db: Database) -> EventLoopFuture<
        Endpoint.Message
    > {
        let readingUsers = MessageReading.query(on: db)
            .filter(\.$message.$id == entity.id!)
            .all()
            .flatMapEach(on: db.eventLoop) { [db] reading -> EventLoopFuture<User> in
                return reading.$user.get(on: db)
            }
            .flatMapEach(on: db.eventLoop) { [db] user in
                Endpoint.User.translate(fromPersistance: user, on: db)
            }
        let sentBy = entity.$sentBy.get(on: db).flatMap {
