...
public protocol UserSocialRepository {
    ...
    func updateRecentlyFollowing(selfUser: Domain.User.ID, groups: [Domain.Group.ID])
        -> EventLoopFuture<Void>
    func resetRecentlyFollowing(selfUser: Domain.User.ID) -> EventLoopFuture<Void>
    func followings(userId: User.ID, selfUser: User.ID, page: Int, per: Int) -> EventLoopFuture<
        Page<GroupFeed>
    >
    ...
    func allUserFeeds(selfUser: User.ID, page: Int, per: Int) -> EventLoopFuture<
        Page<UserFeedSummary>
    >
    func likedUserFeeds(selfUser: User.ID, page: Int, per: Int) -> EventLoopFuture<
        Page<UserFeedSummary>
    >
    ...
    func userLikeLiveCount(selfUser: Domain.User.ID, type: Domain.LiveSeries) -> EventLoopFuture<
        Int
    >
    func getLiveLikedUsers(liveId: Domain.Live.ID, page: Int, per: Int) -> EventLoopFuture<
