...
public protocol MessageRepository {
    func createRoom(selfUser: User.ID, input: CreateMessageRoom.Request) -> EventLoopFuture<MessageRoom>
    ...
    func open(selfUser: User.ID, roomId: MessageRoom.ID, page: Int, per: Int) -> EventLoopFuture<Page<Message>>
