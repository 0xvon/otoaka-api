...
public struct GetUserProfileUseCase: UseCase {
    ...
    public func callAsFunction(_ request: Request) throws -> EventLoopFuture<Response> {
        ...
        return user.flatMap { user -> EventLoopFuture<Response> in
            ...
            let frequentlyWatchingGroups = userSocialRepository.frequentlyWatchingGroups(userId: user.id, selfUser: user.id, page: 1, per: 50).map { $0.items }
            let recentlyFollowingGroups = userSocialRepository.recentlyFollowingGroups(userId: user.id, selfUser: user.id)
            let followingGroups = userSocialRepository.followings(userId: user.id, selfUser: user.id, page: 1, per: 50)
                .map { $0.items }
            let schedule = userSocialRepository.likedLive(userId: user.id, selfUser: user.id, series: .future, page: 1, per: 10)
                .map { $0.items }
