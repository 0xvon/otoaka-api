...
class UserSocialControllerTests: XCTestCase {
    ...
    func testGetUpcomingLives() throws {
        ...
        try app.test(
            .GET, "user_social/upcoming_lives?userId=\(userB.user.id)&page=1&per=10",
            headers: headers
        ) { res in
    ...
    func testGetFollowingUserFeeds() throws {
        ...
        let headers = appClient.makeHeaders(for: userB)
        try app.test(.GET, "user_social/following_user_feeds?page=1&per=10", headers: headers) {
            res in
    ...
    func testLikeLive() throws {
        ...
        try app.test(
            .GET, "user_social/live_liked_users?liveId=\(liveA.id)&page=1&per=10", headers: headers
        ) { res in
            XCTAssertEqual(res.status, .ok, res.body.string)
    ...
    func testGetFollowingPosts() throws {
        ...
        try app.test(
            .GET, "user_social/liked_posts/\(userB.user.id)?page=1&per=10", headers: headers
        ) { res in
    ...
    func testGetLikedLiveTransition() throws {
        ...
        try app.test(
            .GET, "user_social/liked_live_transition?userId=\(user.user.id)",
            ...
            guard let liveParticipatingCount = responseBody.liveParticipatingCount.first else {
                return
            }
    ...
    func testGetRecentlyFollowingGroups() throws {
        ...
        _ = try appClient.updateRecentlyFollowing(
            groups: [groupA.id, groupB.id, groupC.id], with: user)

        try app.test(
            .GET, "user_social/recently_following_groups/\(user.user.id)",
            headers: appClient.makeHeaders(for: user)
        ) { res in
            XCTAssertEqual(res.status, .ok, res.body.string)
