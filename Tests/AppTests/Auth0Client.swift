...
class Auth0Client {
    ...
    func createToken(userName: String, email: String? = nil, password: String = "Passw0rd!!") throws
        -> User
    {
        ...
        let createUserRequest = CreateUserRequest(
            email: email, email_verified: true, username: userName, user_id: userName,
            connection: "Username-Password-Authentication", password: tempPassword)
    ...
    func destroyUser(id: String) throws -> EventLoopFuture<Void> {
        ...
        return app.client.delete(
            "\(domain)/api/v2/users/\(id.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!)",
            headers: headers
        ).map { _ in }
    }
}
